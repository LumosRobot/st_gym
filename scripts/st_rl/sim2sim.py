...
def hydra_mj_config(args_cli) -> Callable:
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            config_path=os.path.abspath(os.path.join('logs', "st_rl",args_cli.experiment_name, args_cli.load_run, "params"))
...
class MujocoSimEnv:
    ...
    def get_obs(self):
        ...
        body_lin_vel_w=[]
        body_pos_w=[]
        expressive_link_name = self.cfg.ref_motion.expressive_link_name
        vel = np.zeros(6)
        if expressive_link_name is not None:
            ...
            body_pos_b = quat_rotate_inverse(base_quat_w.expand(body_pos_b.shape[0], -1), body_pos_b)
            body_lin_vel_b = quat_rotate_inverse(base_quat_w.expand(body_lin_vel_w.shape[0],-1), body_lin_vel_w)
    ...
    def step(self, actions):
        ...
        # updating ref motion
        if hasattr(self,"ref_motion"):
            if(self.ref_motion.frame_idx==self.ref_motion.augment_frame_num-1):
    ...
    def save_log(self, saving_folder):
        ...
        store_action = np.array(self.store_action)[:,0,:]
        store_target_q = np.array(self.store_target_q)[:,0,:]
        store_obs = np.array(self.store_obs)[:,0,:]
        store_ref_motion = np.array(self.store_ref_motion)[:,0,:]
