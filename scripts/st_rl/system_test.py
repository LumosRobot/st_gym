...
def extract_deploy_log(log_file_name):
    ...
    variables_to_extract = [
        ...
        "goal ref_motion", "rl_iter_", "cmd_", "q_", "qd_", "omega", "projected_gravity", "frame_idx"
    ]
...
def plot_joint_timeseries(
    ...
    for plot_idx, joint_idx in enumerate(indices):
        ...
        for data, field_name in zip(data_dicts, fields):
            ...
            assert frame_end < data[field_name].shape[0], \
                f"frame_end: {frame_end} is larger than data[{field_name}] length: {data[field_name].shape[0]}"
