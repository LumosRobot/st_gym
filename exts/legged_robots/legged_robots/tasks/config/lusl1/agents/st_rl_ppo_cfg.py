"""Proximal Policy Optimization (PPO) Configuration for Kepler Lusl1 Robot.

This module defines the configuration for training Lusl1 robot using PPO algorithm
...
Note:
    ...
    specific parameters tuned for Lusl1 robot locomotion tasks.
"""
...
from legged_robots.tasks.config.lusl1.amp_mimic_cfg import *

@configclass
class Lusl1RoughPPORunnerCfg(StRlOnPolicyRunnerCfg):
    """Configuration class for Lusl1 robot PPO training in rough terrain.
    
    This class inherits from StRlOnPolicyRunnerCfg and defines specific
    parameters for training Lusl1 robot using PPO algorithm.
    ...
    experiment_name = "lusl1_rough"            # Experiment identifier
...
@configclass
class Lusl1FlatPPORunnerCfg(Lusl1RoughPPORunnerCfg):
    def __post_init__(self):
        super().__post_init__()

        self.save_interval = 500     # Checkpoint saving frequency
        self.max_iterations = 20000
        self.experiment_name = "lusl1_flat"
        self.algorithm.class_name = algorithm_name  # "PPO" #"TPPO"
        self.policy.class_name = policy_name  # "ActorCritic" #"ActorCriticRecurrent"
        self.runner_class_name = runner_name  # AmpPolicyRunner
        if "A" in self.algorithm.class_name:
